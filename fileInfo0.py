...
headers = ["File Name", "Characters", "Size", "Type", "Modified"]  # column headers
width = [20, 11, 11, 11, 30]  # max width of data in each column
...
def file_info(name):
    ...
    info.append(os.path.getsize(name))
    if os.path.isfile(name):
    	info.append("file")
    if os.path.isdir(name):
    	info.append("Directory")
    info.append(time.ctime(os.path.getmtime(name)))
